...
double tsp_mst(point *V, int n, int *P, graph T)
{
  ...
  for (int i = 0; i < n; i++)
  {
    for (int j = 0; j < n; j++)
    {
      E[ind++] = (edge){.u = i, .v = j, .weight = dist(V[i], V[j])};
    }
  }
  ...
  // on prend le poids de la premiere arête de qsort et on regarde si elle ne forme pas de cycle
  // prendre le premier élément de qsort
  // u c'est i et v c'est j
  int u, v;
  T = createGraph(n);
  for (int i = 0; i < ind; i++)
  {
    for (int j = 0; j < ind; j++)
    {
      ...
      if ((Find(i, parent[i]) == j) && (Find(j + 1, parent[j + 1]) == j) && (Find(j + 1, parent[j + 1]) == i))
      {
        addEdge(T, i, j);
      }
    }
